...
def get_model_name(args):
    if args.backbone.startswith("resnet"):
        add_on = f"{args.resnet_layer}"
    elif args.backbone.startswith("cct"):
        add_on = "none"

    # Get first component of resize if it is a list (e.g. [384,384]) otherwise use the string
    resize = args.resize[1] if isinstance(args.resize, list) else args.resize
    split_aspect = args.aspect_ratio.split("_")
    aspect_ratio = split_aspect[0] 
    name = f"{args.backbone}_{args.aggregation}_{args.random_rotation}_{args.brightness}_{resize}_{args.hard_positives}_{aspect_ratio}_{add_on}"
    return name
