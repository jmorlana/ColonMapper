        ...
        for line in f:
            ...
            if line:
                ...
                if len(node_number) == 2:
                    ...
                    node_number, score, _ = node_number
                    node_number = node_number.strip()
                    score = float(score.strip())
                    scores[image_number] = score
                else:
...
def compute_precision_recall_no_N(ground_truth, localizations):
    ...
    relevant = len(localizations) - skipped # total colon regions 
...
def PR_curve_threshold(models, results):
    ...
    datasets = ['seq2', 'seq3', 'seq4']
    categories = ['single_image', 'single_image_reject','bayesian', 'bayesian_reject']

    dataset_title = {'seq2': 'seq2 vs seq1', 'seq3': 'seq3 vs seq1', 'seq4': 'seq4 vs seq1'}
    ...
    # legend = axs[2].legend(loc='upper left', bbox_to_anchor=(1.05, 1), fontsize=14)  # Adjust the bbox_to_anchor as needed
    # # To prevent the legend from overlapping with the plots, use bbox_inches='tight'
    # plt.tight_layout(rect=[0, 0, 0.85, 1])  # Adjust the rect values as needed

    #plt.legend(loc='upper right')
    plt.tight_layout()
    plt.savefig('c3vd_sequences_final.pdf', format='pdf', dpi=300, bbox_inches='tight')  # Adjust the file name and dpi as needed
    plt.show()
...
def get_metrics(models):
    ...
    results = {}
    node_labels_file = 'labels_seq1.txt'
    for category in categories:
        for model in models:
            key = f'{model}_{category}'
            results[key] = {}
            ...
            for dataset in datasets:
                ...
                localizations_path = join(logs_path, model, category, 'C3VD', localizations_file)
