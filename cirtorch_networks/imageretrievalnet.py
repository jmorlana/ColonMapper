...
from .pooling import MAC, SPoC, GeM, GeMmp, RMAC, Rpool
...
def extract_vectors(net, images, image_size, transform, bbxs=None, ms=[1], msp=1, print_freq=10, central_crop=None):
    # moving network to gpu and eval mode
    prep = time.time()
    net.cuda()
    net.eval()
    #print('>> prepare net', 1000*(time.time()-prep), 'ms')

    loadlist = time.time()
    # creating dataset loader
    loader = torch.utils.data.DataLoader(
        ImagesFromList(root='', images=images, imsize=image_size, bbxs=bbxs, transform=transform, central_crop=central_crop),
        batch_size=1, shuffle=False, num_workers=8, pin_memory=True
    )

    #print('>> load list', 1000*(time.time()-loadlist), 'ms')
    ...
    with torch.no_grad():
        ...
        extraction_noloop = time.time()
        input = next(dataloader_iterator)
        input = input.cuda()
        #vecs_test[:,0] = extract_ss(net, input)
        #print('>> SINGLE EXTRACTION WTITHOUT LOOPS', 1000*(time.time()-extraction_noloop), 'ms')
        ...
        #print('>> Mean time of extraction', 1000*meantime/len(images), 'ms')
        
        print('')
    #print('>> loop', 1000*(time.time()-loop), 'ms')

    #print('>> full extraction', 1000*(time.time()-prep), 'ms')

    return vecs

def extract_vectors_rotated(net, images, image_size, transform, bbxs=None, ms=[1], msp=1, print_freq=10, central_crop=None):
    # moving network to gpu and eval mode
    net.cuda()
    net.eval()
